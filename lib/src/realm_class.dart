...
class Realm implements Finalizable {
  ...
  T add<T extends RealmObject<T>>(T object, {bool update = false}) {
    ...
    createThenAddOrUpdate(object, update);
    return object;
  }
...
extension RealmInternal on Realm {
  ...
  RealmObjectHandle createObject<T>(RealmObject<T> object, RealmObjectMetadata metadata, bool update) {
    ...
    if (update) {
      return realmCore.getOrCreateRealmObjectWithPrimaryKey(this, key, primaryKey.getValue(object));
    }
    return realmCore.createRealmObjectWithPrimaryKey(this, key, primaryKey.getValue(object));
  }
  ...
  RealmList<T>? resolveList<T extends Object?>(ManagedRealmList<T> list) {
    ...
    return RealmListInternal.create<T>(handle, this, list.metadata);
  }
